...
class all_callbacks(object):
    def __init__(self,
        ...
        self.reduce_lr = ReduceLROnPlateau(monitor='val_loss', factor=lr_factor, patience=lr_patience, 
                                mode='min', verbose=verbose, epsilon=lr_epsilon,
        ...
        self.modelcheckperiod=ModelCheckpoint(outputDir+"/KERAS_check_model_epoch{epoch:02d}.h5", verbose=verbose,period=10)
        
        self.modelcheck=ModelCheckpoint(outputDir+"/KERAS_check_model_last.h5", verbose=verbose)

        self.modelcheckweights=ModelCheckpoint(outputDir+"/KERAS_check_model_last_weights.h5", verbose=verbose,save_weights_only=True)
